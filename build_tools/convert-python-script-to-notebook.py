...
In principle, jupytext is enough to convert a Python script (.py file) to a
...
(December 2020) so some of the MyST-specific bits rendering is not great, e.g. admonitions.

This script is taking the option 1. "Find ways to inject raw HTML into generated notebooks" in:
...
def convert_to_html(doc, css_selector=None):
    """Converts docutils document to HTML and select part of it with CSS selector.
    """
...
def admonition_html(doc):
    ...
    Assumes that the docutils document has a single child which is an admonition.
    """
...
def write_without_cell_ids(nb, output_filename):
    ...
    # jupytext writes random cell ids when generating .ipynb from .py, creating unnecessary changes.
